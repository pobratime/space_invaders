...
void game_loop(Game *game) {
  ...
  game->sound_audio_stream =
      SDL_OpenAudioDeviceStream(SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK,
                                &assets.audio.galactic_pulse.spec, NULL, NULL);
  game->music_audio_stream = SDL_OpenAudioDeviceStream(
      SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK, &assets.audio.button_selection.spec,
      NULL, NULL);
  game->sound_audio_stream_2 = SDL_OpenAudioDeviceStream(
      SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK, &assets.audio.laser_gun_classic.spec,
      NULL, NULL);
